...
export class ReviewComponent implements OnInit, OnDestroy {
  ...
  private submitUserOrder(order: Order, total: number, userUid: string) {
    if (this.orderService.addUserOrder(order, total, userUid)) {
      this.cartService.clearCart();
      this.router.navigate(['/order-complete']);
    } else {
      this.openSnackBar('Could not submit order, try again.');
    }
  }

  private submitAnonOrder(order: Order, total: number) {
      if (this.orderService.addAnonymousOrder(order, total)) {
        this.cartService.clearCart();
        this.router.navigate(['/order-complete']);
      } else {
        this.openSnackBar('Could not submit order, try again.');
